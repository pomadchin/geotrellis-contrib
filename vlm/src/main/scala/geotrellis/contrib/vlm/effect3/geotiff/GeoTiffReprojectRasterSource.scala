...
package geotrellis.contrib.vlm.effect3.geotiff

import geotrellis.contrib.vlm._
import geotrellis.contrib.vlm.effect3._
import geotrellis.contrib.vlm.effect3.geotiff.instances._
...
case class GeoTiffReprojectRasterSource[F[_]](
  ...
  def read(extent: Extent, bands: Seq[Int]): F[Raster[MultibandTile]] = {
    ...
    bounds.flatMap(bounds => readBounds(List(bounds), bands).map(_.next))
  }
